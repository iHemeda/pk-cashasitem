...
CreateThread(function()
    while true do
        ...
        if error then
            sleep = 1
            print("~r~[pk-cashasitem] - ERROR - You forgot to add blackmoney in the qbcore config file.~w~")
        end
        Wait(sleep)
    end
end)
...
RegisterNetEvent('pk-cashasitem:server:updateCash', function(id, item, amount, action, display)
    local player = QBCore.Functions.GetPlayer(id)
    if display == nil then
        display = true
    end
